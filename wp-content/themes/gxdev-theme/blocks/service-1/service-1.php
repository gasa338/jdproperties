    ...
    <div class="container mx-auto px-6 relative z-10">
        <div class="max-w-2xl mb-10">
            <?php if (!empty($data['top_title'])) : ?>
                <?php echo _top_title($data['top_title']); ?>
            <?php endif; ?>
            ...
            <?php if (!empty($data['text'])) : ?>
                <div class="maxwell-content mb-2 text-lg <?php echo $color_mode === 'dark_mode' ? ' text-white/60' : 'text-muted-foreground'; ?>"><?php echo apply_filters('the_content', $data['text']); ?></div>
            <?php endif; ?>
        </div>
            ...
            <?php foreach ($data['items'] as $item): ?>
                ...
                <?php else: ?>
                    <div class="group relative p-8 <?php echo $color_mode === 'dark_mode' ? ' bg-white/5' : 'bg-card'; ?> border border-border transition-all duration-500">
                        ...
                        <?php endif; ?>
                        <?php if (!empty($item['link'])): ?>
                            <span class="inline-flex items-center gap-1.5 text-accent mt-5 hover:gap-2.5 transition-all"><?php echo $item['link']['title']; ?>
