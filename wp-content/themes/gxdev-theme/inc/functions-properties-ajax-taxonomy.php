...
add_action('wp_enqueue_scripts', function () {
    ...
    if (! is_page_template('properties-taxonomy.php')) return;
...
/* -------------------------------------------------------------------------- */
/* 2. AJAX handler — filter_properties_taxonomy                                */
/* -------------------------------------------------------------------------- */
add_action('wp_ajax_filter_properties_taxonomy',        'properties_ajax_handler_taxonomy');
add_action('wp_ajax_nopriv_filter_properties_taxonomy', 'properties_ajax_handler_taxonomy');

function properties_ajax_handler_taxonomy()
{

    // Nonce verifikacija
    if (! wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'] ?? '')), 'properties_filter_taxonomy_nonce')) {
    ...
    if (!empty($term_slug) && $_POST['taxonomy'] === 'cat') {
        $prop_category = [$term_slug];
    }else{
        $prop_category = sanitize_text_field($_POST['prop_category'] ?? '');
    }

    if (!empty($term_slug) && $_POST['taxonomy'] === 'jd-type') {
        $prop_types   = [$term_slug];
    } else {
        $prop_types   = array_map('sanitize_text_field', (array) ($_POST['prop_types']  ?? []));
    }

    if (!empty($_POST['term_slug']) && $_POST['taxonomy'] === 'location') {
        $locations = [$_POST['term_slug']];
    } else {
        $locations    = array_map('sanitize_text_field', (array) ($_POST['locations']   ?? []));
    }
    
    if (!empty($term_slug) && $_POST['taxonomy'] === 'other') {
        $other = [$term_slug];
    } else {
        $other = [];
    }
    ...
    if (! empty($prop_types)) {
        ...
        error_log("prop_types: " . json_encode($prop_types));
    }
    ...
    if (! empty($locations)) {
        ...
        error_log("locations in tax_query: " . json_encode($locations));
    }
    ...
    if (! empty($other)) {
        ...
        error_log("other in tax_query: " . json_encode($other));
    }
